...
nevk-yachts-frontend/    ← Customer-facing React SPA (this repository)
...
Deploy the `/dist` folder (output of `npm run build`) to any static hosting provider (Vercel, Netlify, Nginx, etc.). Configure the server to redirect all routes to `index.html` to support client-side routing.
