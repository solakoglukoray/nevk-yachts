...
```
nevk-yachts-backend/     ← API server
nevk-yachts-frontend/    ← Customer-facing React SPA
admin-panel-frontend/    ← Admin management panel (this repository)
```
...
The Axios instance in `src/api/axiosConfig.js` has a **request interceptor** that automatically attaches the `Authorization: Bearer {token}` header to every request — no manual header management needed per call.
